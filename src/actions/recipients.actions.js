...
export const SEND_EMAILS = '[Recipients] SEND_EMAILS';
export function send(data) {
  return { type: SEND_EMAILS, data }
}

export const SEND_EMAILS_SUCCESS = '[Recipients] SEND_EMAILS_SUCCESS';
export function sendSuccess(data) {
  return { type: SEND_EMAILS_SUCCESS, data }
}

export const SEND_EMAILS_FAILED = '[Recipients] SEND_EMAILS_FAILED';
export function sendFailed(error) {
  return { type: SEND_EMAILS_FAILED, error }
}
...
export function sendEmails(data) {
  return async function (dispatch, getState) {
    dispatch(send())
    try {
      let url = `${window.config.API_URL}recipients/send_emails`
      ...
      if (res.status === 201) {
        ...
        dispatch(sendFailed(res.data.message))
        dispatch(updateNotification(res.data.message, 'danger'))
